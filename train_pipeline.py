...
if __name__ == '__main__':
    ...
    if use_wandb:
        ...
        try:
            wandb.init(project="neox_train_pipeline", group=train_args.group_name, name=name, save_code=True,
                       force=False,
                       entity=args.get('wandb', {}).get('team'), settings=wandb_settings)
    ...
    train_dataset = GPT2Dataset(glob_pattern=dset_params["train_path"],
                                ...
                                mode='with_labels',
                                **dset_params)
    eval_every = args.get("evaluate_every", 100)
    ...
    if use_wandb:
        ...
        wandb.watch(model, log_freq=10, log=args.get('wandb', {}).get('watch_model'))
